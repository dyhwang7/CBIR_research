...
def find_harris_corners(input_img, k, window_size, threshold):
    ...
    corner_list = corner_list[:109]
    with open('harris_.csv', mode='w', newline='') as harris_:
        harris_writer = csv.writer(harris_, delimiter=',')
        for i in corner_list:
            harris_writer.writerow(i)
...
def main():
    ...
    #
    img_path = 'test_images/cathedral_700.jpg'
    k = 0.04
    window_size = 3
    threshold = 10000.00
    ...
    if input_img is not None:
        ...
        show_image('harris', mark_keypoints(corner_list, input_img))
    else:
